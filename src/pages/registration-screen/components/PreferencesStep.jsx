    ...
    <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className={`p-4 border rounded-lg text-left transition-all duration-200 hover:shadow-md ${
                localData.subjects.includes(subject.id)
                  ? 'border-primary bg-primary-50 text-primary-700' :'border-border bg-surface text-text-secondary hover:border-primary-300'
              }`}
            >
              <div className="flex items-center space-x-3">
                <Icon 
                  name={subject.icon} 
                  size={20} 
                  className={localData.subjects.includes(subject.id) ? 'text-primary' : 'text-text-tertiary'}
                />
              ...
              <div className="flex items-center justify-between">
                <div>
                  <div className={`font-medium ${
                    localData.experienceLevel === level.value ? 'text-primary-700' : 'text-text-primary'
                  }`}>
                    {level.label}
                  </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className="w-full px-4 py-3 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary transition-colors duration-200 resize-none"
